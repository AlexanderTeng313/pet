...
// 分发内置宠物：随安装包打包在 resources/pets 下（electron-builder extraResources）。
// 别人解压运行后本机 ~/.petpet/pets 是空的，故首次运行自动把内置宠物植入用户目录，
// 免手动导入即可见。仅在目标缺失时复制，绝不覆盖用户已有的自定义宠物。
const BUNDLED_PETS_DIR = app.isPackaged
  ? path.join(process.resourcesPath, 'pets')
  : path.join(__dirname, 'resources', 'pets')

function seedBundledPets() {
  if (!fs.existsSync(BUNDLED_PETS_DIR)) {
    console.log('[PetPet] 无内置宠物包，跳过植入')
    return
  }
  try {
    for (const name of fs.readdirSync(BUNDLED_PETS_DIR)) {
      const src = path.join(BUNDLED_PETS_DIR, name)
      if (!fs.statSync(src).isDirectory()) continue
      const dst = path.join(PETS_ROOT, name)
      if (!fs.existsSync(path.join(dst, 'pet.json'))) {
        fs.cpSync(src, dst, { recursive: true })
        console.log('[PetPet] 已植入内置宠物:', name)
      }
    }
  } catch (e) {
    console.error('[PetPet] 植入内置宠物失败:', e)
  }
}
seedBundledPets()
...
let currentSwitchInterval = 12000  // 2026-09-13 动画切换间隔（托盘菜单可调），默认 12s
let switchIntervalWindow = null   // 2026-09-13 动画切换间隔：自定义输入窗口
...
// 动作按 trigger 分组（用于右键/托盘菜单分区展示，便于区分角色）
// idle=待机动画（休息类，常驻循环） / click=交互动画（单击触发） / menu=菜单动作（仅菜单） / 未声明 trigger 进 other 兜底
function groupedActionMenuItems() {
  const groups = { idle: [], click: [], menu: [], other: [] }
  if (currentPetActions && typeof currentPetActions === 'object') {
    for (const name of Object.keys(currentPetActions)) {
      const act = currentPetActions[name]
      const t = act && act.trigger
      const bucket = groups[t] ? t : 'other'
      groups[bucket].push({ name, label: actionMenuLabel(name, act) })
    }
  }
  return groups
}

// 由分组生成「分区子菜单」模板：待机/交互/菜单各自一个子菜单，未分类兜底；空组不展示
function buildActionGroupSubmenus() {
  const g = groupedActionMenuItems()
  const defs = [
    ['idle', '🛋 待机动画'],
    ['click', '🐾 交互动画'],
    ['menu', '📋 菜单动作'],
  ...
  for (const [key, label] of defs) {
    if (g[key].length) {
      out.push({
        label,
        submenu: g[key].map(a => ({ label: a.label, click: () => send('pet:action', a.name) }))
      })
    }
  }
  return out
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    resizable: false,
    hasShadow: false,
    // 2026-09-13 用户要求：任务栏不显示宠物窗口，只保留右下角托盘图标
    skipTaskbar: true,
    webPreferences: {
  ...
  // 2026-09-13 后台资源优化：窗口最小化/隐藏时暂停动画（省电省 GPU），恢复显示时继续。
  // 仅跟 hide/show（最小化），不跟 blur——否则聚焦提醒/番茄钟子窗口也会把宠物冻住。
  mainWindow.on('hide', () => send('pet:visibility', false))
  mainWindow.on('show', () => send('pet:visibility', true))
}
...
function rebuildTrayMenu() {
  ...
  const template = [
    ...
    // 2026-09-13 动画切换间隔（托盘菜单可调）：radio 显示当前选中 ✓
    {
      label: '⏱ 动画切换',
      submenu: [
        { label: '慢（20 秒）', type: 'radio', checked: currentSwitchInterval === 20000, click: () => setSwitchInterval(20000) },
        { label: '适中（12 秒）', type: 'radio', checked: currentSwitchInterval === 12000, click: () => setSwitchInterval(12000) },
        { label: '快（6 秒）', type: 'radio', checked: currentSwitchInterval === 6000, click: () => setSwitchInterval(6000) },
        { label: '超快（3 秒）', type: 'radio', checked: currentSwitchInterval === 3000, click: () => setSwitchInterval(3000) },
        { label: '✏️ 自定义…', click: () => openSwitchIntervalWindow() }
      ]
    }
  ]
  ...
  // 动作子菜单（从 pet.json 加载，按 trigger 分组：待机/交互/菜单，分区展示便于区分）
  if (currentPetActions && Object.keys(currentPetActions).length > 0) {
    const groupSubs = buildActionGroupSubmenus()
    if (groupSubs.length) {
      template.push({ type: 'separator' })
      template.push(...groupSubs)
    }
  }
  ...
  template.push({ label: '⏰ 提醒我', click: () => openReminderWindow() })
  template.push({ label: '🍅 番茄钟', click: () => openPomodoroWindow() })
...
// 2026-09-13 动画切换间隔：托盘菜单选择后，更新主进程当前值、通知渲染层、刷新菜单「✓」
function setSwitchInterval(ms) {
  currentSwitchInterval = ms
  send('view:switchInterval', ms)
  rebuildTrayMenu()
}

// 渲染层启动后回报已持久化的间隔，使托盘菜单「✓」与用户上次选择一致
ipcMain.on('view:switchIntervalReport', (_e, ms) => {
  if (typeof ms === 'number' && ms >= 1000 && ms <= 86400000) {
    currentSwitchInterval = ms
    rebuildTrayMenu()
  }
})

// 2026-09-13 动画切换间隔：自定义输入窗口（支持秒/分/时）。native Menu 不支持输入框，故独立窗口。
function openSwitchIntervalWindow() {
  if (switchIntervalWindow && !switchIntervalWindow.isDestroyed()) {
    switchIntervalWindow.focus()
    return
  }
  ...
  if (x < work.x) x = Math.min(work.x + work.width - W, pos[0] + (mainWindow ? mainWindow.getSize()[0] : 320) + 8)
  if (y + H > work.y + work.height) y = Math.max(work.y, work.y + work.height - H)
  switchIntervalWindow = new BrowserWindow({
    width: W, height: H,
    x: Math.max(work.x, x), y,
    frame: false, movable: true, resizable: false,
    alwaysOnTop: true, skipTaskbar: true, hasShadow: true,
    webPreferences: {
      preload: path.join(__dirname, 'switch-interval-preload.cjs'),
      contextIsolation: true, nodeIntegration: false, sandbox: true
    }
  })
  switchIntervalWindow.setAlwaysOnTop(true, 'screen-saver')
  switchIntervalWindow.loadFile(path.join(__dirname, 'switch-interval.html'))
  switchIntervalWindow.on('closed', () => { switchIntervalWindow = null })
}
...
// 右键菜单：原生 Menu.popup（自动毛玻璃/定位，替代 HTML 自绘）
// 2026-09-13 Windows：渲染层 e.screenX/screenY 在显示缩放下可能是物理像素，
// 与 popup 期望的 DIP 不一致导致菜单被推向右侧；改用主进程光标坐标（权威 DIP）。
// 右键时光标正好在宠物身上 → 菜单自然跟随宠物。
ipcMain.handle('menu:showContext', (_evt, x, y, dogRect) => {
  const petName = currentPetName || '宠物'
  const template = [
    { label: petName, enabled: false },
    { type: 'separator' },
    { label: '⏰ 提醒我', click: () => openReminderWindow() },
    { label: '🍅 番茄钟', click: () => openPomodoroWindow() },
    { label: `📖 ${petName}日记`, click: () => openDiaryWindow(currentPetId || '') },
    { type: 'separator' }
  ]
  // 2026-09-13 动作收进子菜单：按 trigger 分组（待机/交互/菜单），分区展示便于区分
  if (currentPetActions && Object.keys(currentPetActions).length > 0) {
    const groupSubs = buildActionGroupSubmenus()
    if (groupSubs.length) {
      template.push({ type: 'separator' })
  ...
  // 2026-09-13 关键语义修正：Electron menu.popup({ window, x, y }) 中，
  // 传了 window 时 x/y 是「相对该窗口客户区」的坐标，不是屏幕坐标！
  // 此前按屏幕坐标（窗口位置 + 狗身边界）计算 → Electron 再叠加一次窗口位置，
  // 偏移恰等于窗口位置（表现为一会儿近一会儿远 / 贴屏幕右缘）。
  // 现在：显式传 window（避免默认取"当前聚焦窗口"导致坐标漂移），
  // x/y 用纯客户区坐标 = 狗身边界（渲染层已换算到窗口 CSS 坐标）。
  let mx = null, my = null
  try {
    if (mainWindow && !mainWindow.isDestroyed()) {
      ...
      const GAP = 8, MENU_W = 240, MENU_H = 420
      if (dogRect && Number.isFinite(dogRect.x) && Number.isFinite(dogRect.w) && dogRect.w > 0) {
        // 客户区坐标：狗身体右缘 + GAP，顶部与狗头顶对齐
        let cx = dogRect.x + dogRect.w + GAP
        let cy = dogRect.y
        // 右侧放不下 → 翻到狗左侧
        if (wx + cx + MENU_W > wa.x + wa.width) cx = Math.max(2, dogRect.x - MENU_W - GAP)
        // 下方放不下 → 上移（保持在屏幕工作区内）
        const overflowY = (wy + cy + MENU_H) - (wa.y + wa.height)
        if (overflowY > 0) cy = Math.max(2, cy - overflowY)
        mx = cx
        my = cy
      } else {
        // 兜底：贴窗口右缘（同样是客户区坐标）
        let cx = ww + GAP
        let cy = 0
        if (wx + cx + MENU_W > wa.x + wa.width) cx = Math.max(2, -MENU_W - GAP)
        const overflowY = (wy + cy + MENU_H) - (wa.y + wa.height)
        if (overflowY > 0) cy = Math.max(2, cy - overflowY)
        mx = cx
        my = cy
      }
      console.log('[PetPet][菜单诊断] 窗口屏幕位:', wx, wy, '尺寸:', ww, wh,
        '狗身(客户区):', dogRect ? `${Math.round(dogRect.x)},${Math.round(dogRect.y)} ${Math.round(dogRect.w)}x${Math.round(dogRect.h)}` : '无',
        '→ 弹出(客户区):', Math.round(mx), Math.round(my))
    }
  } catch (e) { console.error('[PetPet][菜单诊断] 锚定失败', e) }
  if (mx == null || my == null) { mx = Math.round(x); my = Math.round(y) } // 最后兜底：渲染层坐标
  menu.popup({ window: mainWindow ?? undefined, x: Math.round(mx), y: Math.round(my) })
})
...
// ---------- 鼠标靠近检测（转头看你·头盯鼠标） ----------
// 2026-09-13：宠物窗口是穿透窗口，渲染层拿不到窗口外的鼠标位置 → 主进程轮询全局光标。
// 光标进入检测圈（窗口中心为圆心、半径 = 窗口对角/2 + MARGIN）时，把归一化偏移 (dx,dy)
// 持续发给渲染层；渲染层按角度把「转头看你」定格到对应帧 → 狗头实时盯着鼠标。
// 2026-09-13 更新：素材本体尺寸不参与检测——只取素材中心作圆心，用【绝对半径】判定
const PROXIMITY_RADIUS_PX = 300     // 进入盯人：距素材中心 300px 内（本体大小不再叠加）
const PROXIMITY_HYST_PX = 80        // 滞后区：已进入后半径再放宽 80px → 边界不会来回抖
const PROXIMITY_EXIT_DELAY_MS = 500 // 退出延迟：连续离开 500ms 才回待机（防边沿抖动导致动作反复切换）
let proxNear = false
let proxOutsideSince = 0
let lastDx = 0
let lastDy = 0
// 狗身体可见范围（渲染层按 alpha 边界上报，窗口客户区坐标）
let dogBox = null
ipcMain.on('pet:dogbox', (_e, box) => { dogBox = box })
function startProximityWatch() {
  setInterval(() => {
    try {
      ...
      const [wx, wy] = mainWindow.getPosition()
      const [ww, wh] = mainWindow.getSize()
      // 圆心 = 素材中心（dogBox 只用来取中心点；没有 dogBox 时退回窗口中心）
      const bx = dogBox ? dogBox.cx : ww / 2
      const by = dogBox ? dogBox.cy : wh / 2
      // 2026-09-13 更新：单一固定半径（不再滞后/延迟）——进出同一个数，超出立即停止
      const radius = PROXIMITY_RADIUS_PX
      const dx = (p.x - wx) - bx
      const dy = (p.y - wy) - by
      const inZone = Math.hypot(dx, dy) <= radius
      const near = inZone // 超出即停止，不再等待延迟
      // 只在"进出状态变化"或"near 且光标移动 ≥1px"时发消息（减少 IPC + 减少无谓帧切换）
      const moved = Math.abs(dx - lastDx) >= 1 || Math.abs(dy - lastDy) >= 1
      if (near !== proxNear || (near && moved)) {
        if (near !== proxNear) {
          console.log('[PetPet][靠近诊断]', near ? '进入' : '离开', '距离:', Math.round(Math.hypot(dx, dy)), '半径:', radius)
        }
        proxNear = near
        lastDx = dx
        lastDy = dy
        send('pet:proximity', { near, dx, dy }) // dx/dy 用原始像素偏移，渲染层据此算角度
      }
    } catch (e) { /* 轮询异常忽略，下一拍继续 */ }
  }, 66)
}

// ---------- 提醒窗口（2026-09-13 独立成窗口） ----------
// 原实现是宠物窗口内的 overlay：最小档下窗口只有 160×206，面板被挤变形、"设置"按钮跑到窗口外点不到
// （实测 ~/.petpet/reminders.json 根本没生成 = 提醒压根没存上）。改为独立窗口，尺寸固定够用。
...
function openReminderWindow() {
  ...
  let y = Math.max(work.y, pos[1])
  if (x + W > work.x + work.width) x = Math.max(work.x, pos[0] - W - 8)
  if (y + H > work.y + work.height) y = Math.max(work.y, work.y + work.height - H)
...
// 2026-09-13 结构化提交（时间 + 重复 + 内容）：不经自然语言解析，直接交给宠物窗口写入
ipcMain.handle('reminder:submitSpec', async (_evt, spec) => {
  if (!mainWindow || mainWindow.isDestroyed()) return { error: '宠物窗口不可用' }
  if (!spec || typeof spec.at !== 'number' || !Number.isFinite(spec.at)) return { error: '时间无效' }
  try {
    return await new Promise((resolve) => {
      const reqId = 'rq' + Date.now() + Math.random().toString(16).slice(2)
      const timer = setTimeout(() => { ipcMain.removeHandler('reminder:parseResult:' + reqId); resolve({ error: '提交超时' }) }, 5000)
      ipcMain.handleOnce('reminder:parseResult:' + reqId, (_e2, r) => { clearTimeout(timer); resolve(r || {}) })
      mainWindow.webContents.send('reminder:parseRequest', {
        reqId,
        spec: { at: spec.at, repeat: spec.repeat || 'none', text: String(spec.text || '该办正事啦！') }
      })
    })
  } catch (e) {
    return { error: '设置失败' }
...
// 解析复用宠物窗口里的 parseReminder（单一实现，避免两套解析结果不一致）：
// 提醒窗口 → 主进程 → 宠物渲染层解析并写入 → 回执
ipcMain.handle('reminder:submitText', async (_evt, text) => {
  if (!mainWindow || mainWindow.isDestroyed()) return { error: '宠物窗口不可用' }
  try {
    const res = await new Promise((resolve) => {
      const reqId = 'rq' + Date.now() + Math.random().toString(16).slice(2)
      const timer = setTimeout(() => { ipcMain.removeHandler('reminder:parseResult:' + reqId); resolve({ error: '解析超时' }) }, 5000)
      ipcMain.handleOnce('reminder:parseResult:' + reqId, (_e2, r) => { clearTimeout(timer); resolve(r || {}) })
      mainWindow.webContents.send('reminder:parseRequest', { reqId, text })
    })
    return res
  } catch (e) {
    return { error: '设置失败' }
...
// ---------- 番茄时钟（2026-09-13 新增） ----------
// 计时器放在主进程：关掉番茄钟窗口也不中断；阶段结束 → 系统通知 + 宠物动作/气泡提醒
const POMODORO_DEFAULT = { workMin: 25, breakMin: 5, longBreakMin: 15, longEvery: 4 }
let pomo = {
  running: false,
  phase: 'work',          // 'work' | 'break'
  remainingMs: POMODORO_DEFAULT.workMin * 60000,
  endAt: 0,
  round: 0,               // 已完成的工作番茄数
  workMin: POMODORO_DEFAULT.workMin,
  breakMin: POMODORO_DEFAULT.breakMin,
  longBreakMin: POMODORO_DEFAULT.longBreakMin,
  longEvery: POMODORO_DEFAULT.longEvery
}
let pomodoroWindow = null

function pomoDurationMs(phase) {
  if (phase === 'work') return pomo.workMin * 60000
  const isLong = pomo.round > 0 && pomo.round % pomo.longEvery === 0
  return (isLong ? pomo.longBreakMin : pomo.breakMin) * 60000
}

function pomoSnapshot() {
  const remain = pomo.running ? Math.max(0, pomo.endAt - Date.now()) : pomo.remainingMs
  return {
    ...
    longBreakMin: pomo.longBreakMin
  }
}

function pomoPushState() {
  const snap = pomoSnapshot()
  if (pomodoroWindow && !pomodoroWindow.isDestroyed()) {
    pomodoroWindow.webContents.send('pomodoro:state', snap)
  }
  send('pomodoro:state', snap) // 宠物窗口：头顶气泡显示倒计时（常驻，暂停/停止时隐藏）
}

function pomoStart() {
  if (pomo.running) return
  if (pomo.remainingMs <= 0) pomo.remainingMs = pomoDurationMs(pomo.phase)
...
function pomoReset() {
  ...
  pomo.phase = 'work'
  pomo.round = 0
  pomo.remainingMs = pomoDurationMs('work')
  pomoPushState()
}
...
// 阶段推进：completed=true 表示自然走完（要提醒），false 表示手动跳过（静默）
function pomoAdvance(completed) {
  ...
  if (completed) {
    if (finished === 'work') {
      ...
      try {
        new Notification({
          title: '🍅 番茄完成',
          body: `第 ${pomo.round} 个番茄结束，休息一下～`
        }).show()
      } catch (e) { /* 无通知权限忽略 */ }
      send('pomodoro:phase', { done: 'work', round: pomo.round })
    } else {
      try {
        new Notification({ title: '☕ 休息结束', body: '回来开工啦！' }).show()
      } catch (e) { /* 忽略 */ }
      send('pomodoro:phase', { done: 'break', round: pomo.round })
    }
  }
  pomo.phase = finished === 'work' ? 'break' : 'work'
  pomo.remainingMs = pomoDurationMs(pomo.phase)
  pomo.endAt = Date.now() + pomo.remainingMs
  pomoPushState()
}
...
setInterval(() => {
  if (!pomo.running) return
  if (Date.now() >= pomo.endAt) pomoAdvance(true)
  else pomoPushState()
}, 1000)

ipcMain.handle('pomodoro:get', () => pomoSnapshot())
// 2026-09-13 动画切换间隔：自定义输入窗口 IPC
ipcMain.handle('switchinterval:get', () => currentSwitchInterval)
ipcMain.on('switchinterval:apply', (_e, ms) => {
  if (Number.isFinite(ms) && ms >= 1000 && ms <= 86400000) { // 1 秒 ~ 24 小时
    setSwitchInterval(ms)
    if (switchIntervalWindow && !switchIntervalWindow.isDestroyed()) switchIntervalWindow.close()
  }
})
ipcMain.on('switchinterval:close', () => {
  if (switchIntervalWindow && !switchIntervalWindow.isDestroyed()) switchIntervalWindow.close()
})
ipcMain.on('pomodoro:start', () => pomoStart())
ipcMain.on('pomodoro:pause', () => pomoPause())
ipcMain.on('pomodoro:reset', () => pomoReset())
ipcMain.on('pomodoro:skip', () => pomoSkip())
ipcMain.on('pomodoro:set', (_e, cfg) => {
  if (!cfg) return
  if (Number.isFinite(cfg.workMin)) pomo.workMin = Math.max(1, Math.min(180, Math.round(cfg.workMin)))
  if (Number.isFinite(cfg.breakMin)) pomo.breakMin = Math.max(1, Math.min(60, Math.round(cfg.breakMin)))
  if (Number.isFinite(cfg.longBreakMin)) pomo.longBreakMin = Math.max(1, Math.min(120, Math.round(cfg.longBreakMin)))
  if (!pomo.running) pomo.remainingMs = pomoDurationMs(pomo.phase)
  pomoPushState()
})
...
function openPomodoroWindow() {
  ...
  pomodoroWindow = new BrowserWindow({
    ...
    resizable: false,
    alwaysOnTop: true,
    skipTaskbar: true, // 任务栏不显示，只走托盘/菜单入口
    hasShadow: true,
    webPreferences: {
      preload: path.join(__dirname, 'pomodoro-preload.cjs'),
      contextIsolation: true,
      nodeIntegration: false,
      sandbox: true
    }
  ...
  pomodoroWindow.webContents.on('did-finish-load', () => pomoPushState())
}
...
ipcMain.handle('pet:list', () => {
  try {
    const dirs = fs.readdirSync(PETS_ROOT, { withFileTypes: true })
      ...
      .sort() // 确定性排序：让 `coke` 稳定排在 demo 前面成为默认宠物
    return dirs
  } catch (e) {
    return []
  }
...
function fireReminder(id) {
  ...
  // 2026-09-13：横幅在最小档窗口里又小又容易错过 → 一律补发系统通知（窗口可见时也发，双保险）
  try {
    new Notification({ title: '⏰ ' + (currentPetName || '宠物') + '提醒你', body: r.text }).show()
  } catch (e) { /* 无通知权限时忽略 */ }
  send('reminder:fire', { id, text: r.text })
  // 提醒到点：把宠物窗口顶到最前，避免提醒"静悄悄"
  try {
    if (mainWindow && !mainWindow.isDestroyed()) {
...
app.whenReady().then(() => {
  ...
  createWindow()
  startProximityWatch()  // 2026-09-13：鼠标靠近检测（转头看你）
